		...
		for(int i=0; i<tokens.size(); i++) {
			if(i == tokens.size()-1) {
				if(isWord(tokens.get(i))){
					...
					numWords++;
					numSentences++;		//some refactoring can be applied here 
				}						//putting sentence after for loop etc..
				else
					numSentences++;
			}
			else if(isWord(tokens.get(i)) ) {
//				System.out.println("word : " + tokens.get(i));
				numSyllables += countSyllables(tokens.get(i));
				numWords++;
			}
			else {
...
//		System.out.println("syl : "+numSyllables + "\nword : " +numWords + "\nsent : "+ numSentences);
	}
	...
	public static void main(String[] args)
	{
		...
		new EfficientDocument("Time to get weeeeeeeeeeeird, with, the. Punctuation");
		System.out.println();
